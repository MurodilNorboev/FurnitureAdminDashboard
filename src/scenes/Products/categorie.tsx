...
//     try {
//       const response = await axios.get<{ data: Todo[] }>(`${baseAPI}/product/all`);
...
//     const token = localStorage.getItem('token');
//     try {
//       const { data } = await axios.delete<any>(`${baseAPI}/product/delete/` + id, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       if (data.success) {
//         getTodo();
...
//         .map((id, ind) => {
//           const todo = data.find((item) => item._id === id);
//           return todo ? [todo.Popular, todo.label, todo.image, todo.sana, todo._id] : [];
//         })
//         .filter((row) => row.length > 0);
...
//                 </td>
//                 <td>
//                   <Typography level="body-xs">{row.Popular}</Typography>
//                 </td>
//                 <td>{row.label}</td>
//                 <td>
//                   <img
