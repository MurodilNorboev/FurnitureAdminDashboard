...
import { Campingimges } from '../mock/campingdata';
import CampingPlaceDOWN from '../download/campingPlaceDOWN';

const CampingPlaceComponent: React.FC = () => {
  const [data, setData] = useState< CampcarCamp[]>(Campingimges);
  ...
  return (
    <>
      <Box sx={{ alignItems: "center", display: 'flex', mb: 1, gap: 1, justifyContent: 'space-between' }}>
        <Typography level="h2">Camping Place</Typography>
        <Box sx={{ display: 'flex', gap: 1, py: 2 }}>
          {error && <Typography color="danger">{error}</Typography>}
          <div style={{ display: "flex", gap: "20px" }}>
            <Button onClick={handleDownloadPDF}>Download PDF</Button>
            <CampingPlaceDOWN onAdd={(newCar) => setData((prev) => [...prev, newCar])} /> {/* onAdd propini berish */}
          </div>
        </Box>
      </Box>
        ...
        <Table stickyHeader hoverRow>
          <thead>
            <tr>
              ...
              <th>Location</th>
              <th>Brand</th>
              <th style={{paddingLeft:'12px'}}>info</th>
              <th>Delet</th>
            </tr>
          </thead>
          <tbody>
            {data.map((row) => (
              <tr key={row.id}>
                ...
                <td><Typography level="body-xs">{row.campingName}</Typography></td>
                <td><Typography level="body-xs">{row.location}</Typography></td>
                <td><Typography level="body-xs">{row.number}</Typography></td>
