...
import UsedCarDOWN from '../download/usedCarDOWN';
import { usedcarCars } from '../mock/usedCardata';

const UsedCarComponent: React.FC = () => {
  const [data, setData] = useState<CampcarMoto[]>(usedcarCars);
  ...
  const handleRowClick = (motorId: number) => {
    navigate(`/omaterial/usedCar/${motorId}`);
  };

  return (
    <>
      <Box sx={{ alignItems: "center", display: 'flex', mb: 1, gap: 1, justifyContent: 'space-between' }}>
        <Typography level="h2">Used Car</Typography>
        <Box sx={{ display: 'flex', gap: 1, py: 2 }}>
          {error && <Typography color="danger">{error}</Typography>}
          <div style={{ display: "flex", gap: "20px" }}>
            <Button onClick={handleDownloadPDF}>Download PDF</Button>
            <UsedCarDOWN onAdd={(newCar) => setData((prev) => [...prev, newCar])} /> {/* onAdd propini berish */}
          </div>
        </Box>
      </Box>
