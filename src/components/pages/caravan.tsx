...
import CarvanDOWN from '../download/caravanDOWN';
import { caravanCars } from '../mock/caravandata';

const CaravanComponent: React.FC = () => {
  const [data, setData] = useState<CampcarMoto[]>(caravanCars);
  ...
  const handleRowClick = (carvanId: number) => {
    navigate(`/omaterial/caravan/${carvanId}`);
  };

  return (
    <>
      <Box sx={{ alignItems: "center", display: 'flex', mb: 1, gap: 1, justifyContent: 'space-between' }}>
        <Typography level="h2">Caravan</Typography>
        <Box sx={{ display: 'flex', gap: 1, py: 2 }}>
          {error && <Typography color="danger">{error}</Typography>}
          <div style={{ display: "flex", gap: "20px" }}>
            <Button onClick={handleDownloadPDF}>Download PDF</Button>
            <CarvanDOWN onAdd={(newCar) => setData((prev) => [...prev, newCar])} /> {/* onAdd propini berish */}
          </div>
        </Box>
      </Box>
